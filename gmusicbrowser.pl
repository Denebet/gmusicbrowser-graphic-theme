...
(	{ label => _"Song Properties",	code => sub { DialogSongProp (@{ $_[0]{IDs} }); },	onlyone => 'IDs', stockicon => 'gtk-edit' },
	...
	{ label => _"Play Only Selected",code => sub { Select(song => 'first', play => 1, staticlist => $_[0]{IDs} ); },
		onlymany => 'IDs', 	stockicon => 'gmb-media-play-list'},
	{ label => _"Play Only Displayed",code => sub { Select(song => 'first', play => 1, staticlist => \@{$_[0]{listIDs}} ); },
		test => sub { @{$_[0]{IDs}}<2 }, notmode => 'A',	onlymany => 'listIDs',	stockicon => 'gmb-media-play-list' },
...
our @TrayMenu=
(	{ label=> sub {$::TogPlay ? _"Pause" : _"Play"}, code => \&PlayPause,	stockicon => sub { $::TogPlay ? 'gmb-media-pause-list' : 'gmb-media-play-list'; }, id=>'playpause' },
	{ label=> _"Stop", code => \&Stop,	stockicon => 'gmb-media-stop-list' },
...
#	warn $_ for @paths;
#	warn Gtk3::IconTheme::get_default->lookup_icon('gmb-media-play-list',48,[''])->get_filename;
...
INIT
{ %PPSQ_Icon=
 (	play	=> ['gmb-media-play-list', '<span font_family="Sans">▶</span>'],
	pause	=> ['gmb-media-pause-list','<span font_family="Sans">▮▮</span>'],
	stop	=> ['gmb-media-stop-list', '<span font_family="Sans">■</span>'],
 );
}
...
{	my ($ID,$notcurrent,$text)=@_;
	...
	{	my $n;
		...
		{	my $max= $NBQueueIcons||1000;
			$max= @$Queue unless $max < @$Queue;
			$n= first { $Queue->[$_]==$ID } 0..$max-1;
		}
		if ($text) { $status= defined $n ? "<b>ᐅ<sup><small>".($n+1)."</small></sup></b>" : "<b>ᐅ<sup><small>+</small></sup></b>"; }
...
{	my ($listref,$return)=@_;
	...
	my $m=int($sec/60); $sec=sprintf '%01d',$sec%60;
	my $h=int($m/60);     $m=sprintf '%01d',$m%60;
	my $nb=@$listref;
...
{	my $vbox=Gtk3::VBox->new(FALSE, 2);
	...
	#packing
	$vbox->pack_start($_,FALSE,FALSE,1) for @layouts_combos,$reloadlayouts,$checkT1,$checkT2;
	$vbox->set_border_width(WINDOW_PADDING);
	return $vbox;
}
