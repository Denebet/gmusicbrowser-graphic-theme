...
(	Prev =>
	...
	{	class	=> 'Layout::Button',
		...
		stock	=> {pause => 'gmb-media-pause', play => 'gmb-media-play' },
	...
	Check =>
	{	class	=> 'Layout::Button',
		activate	=> "RunPerlCode(::IdleCheck)",
	},
	Scan =>
	{	class	=> 'Layout::Button',
		activate	=> "RunPerlCode(::IdleScan)",
	},
	Quit =>
	{	class	=> 'Layout::Button',
		stock	=> 'gtk-quit',
	...
	{	class	=> 'Layout::Button',
		...
		stock	=> { random => 'gmb-random', shuffle => 'gmb-shuffle', sorted => 'gmb-sort-ascending' },
	...
	{	class	=> 'Layout::Button',
		...
		stock	=> sub { 'gmb-vol'.( $_[0] eq 'm' ? 'm' : int(($_[0])/100*$::NBVolIcons) );  },
...
{	my $nopopup= $_[0];
	...
	{	my $submenu=Gtk3::Menu->new;
		...
		#$item->set_draw_as_radio(1);
		$sitem->set_submenu($submenu);
		$sitem->set_name('menuitem-savedlists');
		$menu->prepend($sitem);
	}
	return $menu if $nopopup;
	::PopupMenu($menu);
...
{	my ($class,$layout,%options)=@_;
	my @original_args=@_;
	my $fallback=delete $options{fallback} || 'Lists_Library_Context';
	my $opt0={};
	if (my $opt= $layout=~m/^[^(]+\(.*=/)
...
{	my ($class,$layout,$widget)=@_;
	...
	my $self=Layout::Window::new($class,$layout, wintype=>'popup', 'pos'=>undef, size=>undef, fallback=>'Full_with_buttons', popped_from=>$widget);
...
(	{ label => _"New list",		code => sub { $_[0]{self}->newtab('EditList',1,{songarray=>''}); },	type=> 'L', stockicon => 'gtk-add', },
	...
	{ label => _"Open Playlist",	code => sub { $_[0]{self}->newtab('@PlayList',1); },			type=> 'L', stockicon => 'gmb-media-play',
...
{	my ($class,$opt)=@_;
	...
	{	my $button= ::NewIconButton('gmb-add');
...
(	{ stockicon=> 'gmb-view-list',	label=>_"Show file list",  toggleoption=>'self/show_list',  cb=> sub { $_[0]{self}->update_showhide; }, },
	...
	{ stockicon=> 'gmb-fullscreen',	label=>_"Fullscreen",	cb=> sub { $_[0]{view}->set_fullscreen(1); },	},
...
{	my ($self,$nb)=@_;
	...
	$self->set_tooltip_text(_("Song rating:")." $nb"._(" %"));
